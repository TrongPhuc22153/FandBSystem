...
import useUserProductReview from "../../../reviews/hooks/useUserProductReview";
...
import RatingStar from "../../../reviews/components/RatingStar/RatingStar";
import { hasRole } from "../../../../shared/utils/authUtils";
import { ROLES } from "../../../../shared/constants/roles";
import { useModal } from "../../../../shared/context/ModalContext";
import ImagesShowcase from "../../../images/components/ImageShowcase/ImageShowcase";
import { INSUFFICIENT_AUTHENTICATION_ERROR } from "../../../../shared/constants/error";
import useSignedUser from "../../../users/hooks/useSignedUser";
import {useDispatch, useSelector} from "react-redux";
import {resetAddState} from "../../../cart/slices/cartSlice";
import {addToCart} from "../../../cart/thunks/cartThunk";
...
const SingleProductPage = () => {
  ...
  const {
    success: addSuccess,
    error: addError
  } = useSelector((state) => state.cart.add);
  const dispatch = useDispatch();

  const { review , mutate: mutateUserRating } =
      useUserProductReview({ productId });
    ...
    if (addError) {
      if(addError.error === INSUFFICIENT_AUTHENTICATION_ERROR){
        onOpen({
          ...
          onNo: () => dispatch(resetAddState()),
        });
      }else{
        showNewAlert({
          message: addError.message || "Error while adding new item",
          variant: "danger",
          action: () => dispatch(resetAddState()),
        });
        dispatch(resetAddState());
      }
    }
  }, [addError, showNewAlert, dispatch, onOpen, navigate]);

  const handleQuantityChange = useCallback((newQuantity) => {
    setQuantity(newQuantity);
  }, []);

  const handleAddToCart = useCallback((productId, qty) => {
      dispatch(addToCart({
        productId: productId,
        quantity: qty,
      }));
          ...
          <div className="row mt-5">
            <div className="tabs-listing">
              ...
              <div className="tab-content py-5" id="nav-tabContent">
                ...
                >
                  {hasRole(user, ROLES.CUSTOMER) && review && (
                    <ReviewForm
                      review={review}
                      productId={productId}
                      handleCreateOrUpdateUserRating={handleCreateOrUpdateUserRating}
                    />
                  )}
                  <ReviewList
                      reviews={reviews}
