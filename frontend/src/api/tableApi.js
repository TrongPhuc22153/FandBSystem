...
export const fetchAvailableTables = async ({
  ...
  const response = await fetch(
    `${RESERVATION_TABLES_AVAILABILITY_ENDPOINT}?${params.toString()}`
  );
  if (!response.ok) {
    throw await response.json();
  }
...
export const fetchTableStatusSummary = async ({ date, time }) => {
  ...
  const response = await fetch(
    `${RESERVATION_TABLES_SUMMARY_ENDPOINT}?${params.toString()}`
  );
...
// Update table status
export const updateReservationTableStatus = async ({
  id,
  status,
  isDeleted,
  token,
