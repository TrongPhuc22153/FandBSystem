...
export function RecommendedMenu() {
  ...
  useEffect(() => {
    const updateDimensions = () => {
      ...
      slickRef.current.style.transform = `translateX(-${
        selectedCategory * tabMenuWidth
      }px)`;
  ...
  const onSelectCategory = (index) => {
    slickRef.current.style.transition =
            ...
            <div className="tab-menu" ref={tabMenuRef}>
              <div className="slider slider-nav">
                {foodsPerCategory.map((category, index) => (
                  <div
                    key={index}
                    className={`tab-title-menu flex-grow-1 cursor-pointer ${
                      selectedCategory == index ? "is-active" : ""
                    }`}
