...
import styles from "./ReservationItemsTable.module.css";
import {
  ORDER_ITEM_STATUSES,
...
export function ReservationItemsTable({
  reservation,
  isLoading,
  onCancelReservationItem,
  onServeReservationItem,
}) {
  return (
    <div>
      <h4 className={styles.sectionTitle}>Reservation Items</h4>
      {isLoading ? (
        <div className={styles.loading}>Loading items...</div>
      ) : reservation && reservation.menuItems?.length > 0 ? (
        <div className="table-responsive">
          <table className="table table-striped" aria-label="Reservation items">
            <thead>
              <tr>
                <th>Item</th>
                <th>Quantity</th>
        ...
        <div className={styles.emptyState}>No active reservation items</div>
      )}
    </div>
  );
}
