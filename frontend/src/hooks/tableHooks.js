...
import {
    ...
    fetchReservationTableByName,
    updateReservationTable,
    createReservationTable,
    createReservationTablesBulk,
    deleteReservationTable,
    updateReservationTableStatus,
} from '../api/tableApi'; // Assuming your API functions are in this file
...
// Hook to fetch a single reservation table by name
export const useReservationTableByName = ({ name }) => {
    return useSWR(name ? `reservation-table-name-${name}` : null, () =>
        fetchReservationTableByName({ name })
    );
};

// Hook to fetch all reservation tables with pagination
export const useReservationTables = ({ page = 0, size = 10, direction = SORTING_DIRECTIONS.ASC, field = "tableNumber", isDeleted = false, status, tableNumber } = {}) => {
    return useSWR([RESERVATION_TABLES_ENDPOINT, page, size, direction, field, isDeleted, status, tableNumber],
        () => fetchReservationTables({ page, size, direction, field, isDeleted, status, tableNumber })
    );
};
...
export const useReservationTableActions = () => {
    ...
    const handleUpdateReservationTable = useCallback(
        async (id, requestData) => {
            ...
            try {
                ...
                setUpdateSuccess(response?.message || "Reservation table updated successfully");
    ...
    const handleUpdateReservationTableStatus = useCallback(
        async ({ id, status, isDeleted }) => {
            ...
            try {
                const response = await updateReservationTableStatus({ id, status, isDeleted, token });
                setUpdateStatusSuccess(response?.message || "Reservation table updated successfully");
                return response;
            } catch (error) {
